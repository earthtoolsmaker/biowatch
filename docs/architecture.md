...
- **Responsibilities**:
  ...
  - Auto-updates
  - Theme service (wraps `nativeTheme`, persists user preference to
    `userData/preferences.json`, broadcasts `theme:changed` to renderers).
...
| ------------------------ | ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sequences-worker`       | `src/main/services/sequences/worker.js`               | Sequence-aware analytics (species distribution, timeseries, heatmap, daily activity, best-media, gallery pagination, per-deployment composition). Spawned per request via `runInWorker`. |
