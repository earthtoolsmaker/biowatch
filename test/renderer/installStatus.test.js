...
describe('getModelInstallStatus', () => {
  test("returns 'installed' when both model and env are present", () => {
    ...
    assert.equal(getModelInstallStatus(SPECIESNET, installedModels, installedEnvs), 'installed')
  })

  test("returns 'env-missing' when model is present but env is not", () => {
    const installedModels = [{ id: 'speciesnet', version: '4.0.1a' }]
    const installedEnvs = []
    assert.equal(getModelInstallStatus(SPECIESNET, installedModels, installedEnvs), 'env-missing')
  })

  test("returns 'not-installed' when model is not present", () => {
    const installedModels = []
    const installedEnvs = [{ id: 'common', version: '0.1.4' }]
    assert.equal(getModelInstallStatus(SPECIESNET, installedModels, installedEnvs), 'not-installed')
  })
  ...
  test('returns the right status for a different model in the same env', () => {
    ...
    assert.equal(getModelInstallStatus(DEEPFAUNE, installedModels, installedEnvs), 'not-installed')
  })
})
